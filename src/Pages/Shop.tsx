...
export function Shop() {
  ...
  const [visibleCount,setVisibleCount]=useState(24); const loadMoreRef=useRef<HTMLDivElement>(null);
  ...
  const visibleProducts=(products??[]).slice(0,visibleCount); const hasMore=visibleCount<(products?.length??0);
  useEffect(()=>setVisibleCount(24),[products]);
  useEffect(()=>{if(!hasMore||!loadMoreRef.current)return;const observer=new IntersectionObserver((entries)=>{if(entries[0]?.isIntersecting)setVisibleCount(c=>Math.min(c+24,products?.length??c));},{rootMargin:"600px 0px"});observer.observe(loadMoreRef.current);return()=>observer.disconnect();},[hasMore,products?.length]);
  return <div className="mx-auto max-w-7xl px-5 py-10 sm:px-8 lg:px-10">
    ...
    <div className="mt-8 grid gap-10 lg:grid-cols-[220px_1fr]">
      ...
      <div>{isLoading?<div className="grid grid-cols-2 gap-x-4 gap-y-10 md:grid-cols-3 md:gap-x-6">{Array.from({length:6}).map((_,i)=><div key={i} className="aspect-[4/5] animate-pulse bg-soft"/>)</div>:products?.length?<><div className="grid grid-cols-2 gap-x-4 gap-y-10 md:grid-cols-3 md:gap-x-6">{visibleProducts.map(p=><ProductCard key={p.id} product={p}/>)}</div>{hasMore&&<div ref={loadMoreRef} className="flex min-h-28 items-center justify-center pt-10"><span className="text-xs text-mut">Loading more rugs…</span></div>}</>:<div className="border border-line px-6 py-20 text-center"><h2 className="text-2xl">No rugs found</h2><p className="mt-2 text-sm text-mut">Try clearing a filter and explore the full collection.</p></div>}</div>
    </div>
  </div>;
}
