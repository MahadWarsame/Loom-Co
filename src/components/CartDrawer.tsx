  ...
  return (
    <>
      ...
      <aside
        ...
        aria-hidden={!open}
        className={`fixed right-0 top-0 z-[70] flex h-dvh w-full max-w-md flex-col border-l border-line bg-bg shadow-2xl transition-transform duration-300 ease-out ${open ? "translate-x-0" : "translate-x-full"}`}
      >
          ...
          <div className="flex flex-1 flex-col items-center justify-center px-8 text-center">
            <div className="grid h-16 w-16 place-items-center rounded-full bg-soft">
              <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" className="h-7 w-7">
                <path d="M4 5h2l1.6 10.2a2 2 0 0 0 2 1.8h7.7a2 2 0 0 0 2-1.6L21 8H7" />
                  ...
                  <div key={item.variantId} className="flex gap-3 px-5 py-5 sm:px-6">
                    ...
                    <div className="min-w-0 flex-1">
                      ...
                      <div className="mt-2 flex items-baseline gap-2 text-sm">
                        <b>{sek(item.price)}</b>
                        {item.price < item.regularPrice && <s className="text-[11px] text-mut">{sek(item.regularPrice)}</s>}
                      </div>
